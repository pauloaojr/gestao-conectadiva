...
interface AssignTimeSlotModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (attendants: Array<{ id: string; name: string }>) => void;
  timeSlot?: { time: string; days: string[] };
  existingAssignments?: Array<{
    attendantId: string;
    timeSlots: Array<{ time: string; days: string[] }>;
  }>;
}

export const AssignTimeSlotModal = ({
  open,
  onOpenChange,
  onSubmit,
  timeSlot,
  existingAssignments = []
}: AssignTimeSlotModalProps) => {
  ...
  const isAttendantAlreadyAssigned = (userId: string, profileId: string) => {
    if (!timeSlot) return false;

    // Verificar usando user_id (que é usado nas atribuições) ou profile id (para compatibilidade)
    const assignment = existingAssignments.find(a =>
      a.attendantId === userId || a.attendantId === profileId
    );
    if (!assignment) return false;

    return assignment.timeSlots.some(slot =>
      slot.time === timeSlot.time &&
      slot.days.some(day => timeSlot.days.includes(day))
    );
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Atribuir Horário</DialogTitle>
          <DialogDescription className="sr-only">
            Selecione os atendentes para atribuir a este horário.
          </DialogDescription>
          {timeSlot && (
            <p className="text-sm text-gray-600">
              Horário: {timeSlot.time} - Dias: {formatDays(timeSlot.days)}
            </p>
          )}
        </DialogHeader>
            ...
            {isLoading ? (
              ...
              <div className="grid gap-2">
                {users.filter(user => user.is_active && user.role === 'user').map((user) => {
                  const isAlreadyAssigned = isAttendantAlreadyAssigned(user.user_id, user.id);
